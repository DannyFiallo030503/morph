...
- **Phase 0 – Stable Core**
  Game loop, responsive canvas, basic entities, events, input (keyboard + mouse). Simple image and sound loading. A "bouncing ball" example with no modules.

- **Phase 1 – Foundational modules**
  `tile-engine` (load and render 2D tile maps), `physics-2d` (AABB collisions, gravity), `camera-2d` (follow, zoom).

- **Phase 2 – First preset (Platformer)**
  Combine the three modules above. Simple platformer mini‑game (collect coins, jump on enemies).

- **Phase 3 – Advanced modules**
  `procedural` (noise, infinite maps), `isometric` (2D → isometric transformation), `soul-combat` (combat system).

- **Phase 4 – Genre presets**
  Turn‑based RPG, Soul‑like, Open world.
